...
import { ArrowRight, Eye, GitBranch, Lock, Shield, Terminal, Zap } from 'lucide-react';

const featureCards = [
  {
    icon: Lock,
    title: 'Auth0 Token Vault',
    desc: 'Your OAuth tokens stay inside Auth0. Sanctum only requests scoped access when it needs it.',
    badgeClassName: 'bg-violet-600/20 border-violet-500/30',
    iconClassName: 'text-violet-400',
  },
  {
    icon: Zap,
    title: 'Authenticated RAG',
    desc: 'The agent indexes only the data it can fetch with your active connections and keeps stores isolated per user.',
    badgeClassName: 'bg-indigo-600/20 border-indigo-500/30',
    iconClassName: 'text-indigo-400',
  },
  {
    icon: Eye,
    title: 'Step-up Auth Gates',
    desc: 'Every write action is staged first. You review it, re-verify if needed, and then approve execution.',
    badgeClassName: 'bg-fuchsia-600/20 border-fuchsia-500/30',
...
const steps = [
  {
    ...
    desc: 'Sign in with Auth0 and connect Gmail and GitHub. Token Vault stores the OAuth tokens securely so Sanctum never touches raw credentials.',
  },
  {
    step: '02',
  ...
  {
    step: '04',
    title: 'Approve before acting',
    desc: 'If the agent drafts an email or GitHub comment, it pauses for your approval and can require fresh authentication before any write happens.',
  },
];
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-8 py-24 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <p className="text-xl text-white/50 max-w-2xl mb-12 leading-relaxed">
            Sanctum connects your AI agent to Gmail and GitHub using Auth0 Token Vault
            ...
            <div className="grid grid-cols-3 gap-4 relative">
              ...
              {featureCards.map((item) => (
                ...
                >
                  ...
                  />
                  <div className={`w-10 h-10 rounded-xl border flex items-center justify-center mb-4 ${item.badgeClassName}`}>
